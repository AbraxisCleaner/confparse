...
#define PRINT_TABS(buf, num) for (auto i = num; i != 0; --i) { *buf = '\t'; ++buf; }
#define BUFGO(buf, ch) while (*buf != ch) ++buf
...
#if defined(IC_INI) || defined(IC_ALL)
...
#if defined(IC_XML) || defined(IC_ALL)
...
#if defined(IC_JSON) || defined(IC_ALL)
...
#if defined(IC_YAML) || defined(IC_ALL)
